...
$(document).ready(function() {
    ...
    $(document).on("click", "#saveQA", function() {
        ...
        $.ajax({
            method: "post",
            url: $("#feDetId").val() === "" ? "question/save" : $("#feDetId").val()+"/save",
            data: formData,
            contentType: false,
            processData: false,
...
function loadLevels(){
    ...
    var callback = function(result){
        ...
        if ($("#levelValue").val() !== "" || $("#levelValue").val() !== undefined) {
            $("#difficultLevel").val($("#levelValue").val());
        }
    };
    //CALL AJAX
...
function isValidAddQuestion() {
    ...
    if ($("#difficultLevel").val() === "") {
        validateText += "<li>กรุณาเลือกระดับความยาก</li>"
    }
    for (var key of Object.keys(ckeditorMap)) {
